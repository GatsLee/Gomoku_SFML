...
SelectAIRuleScene::SelectAIRuleScene(sf::RenderWindow* window)
    : AScene(SELECT_AI_RULE, window)
{}

SelectAIRuleScene::~SelectAIRuleScene()
{}

void SelectAIRuleScene::Init()
{
    //set Sprites
...
void SelectAIRuleScene::Update(const sf::Vector2i& mousePosition, std::vector<AScene *>* mScenes, sf::Event event)
{
    if (IsAnySceneRunning(mScenes) == false)
    {
        ...
        {
            SetNextSceneType(AScene::SELECT_AI_STONE);
            PlayAIScene::AIRuleSetting = 0;
            SetIsRunning(false);
        }
        else if (mStandardRuleButton->getState() == Button::ACTIVE)
        {
            SetNextSceneType(AScene::SELECT_AI_STONE);
            PlayAIScene::AIRuleSetting = 1;
            SetIsRunning(false);
        }
        else if (mRenjuRuleButton->getState() == Button::ACTIVE)
        {
            SetNextSceneType(AScene::SELECT_AI_STONE);
            PlayAIScene::AIRuleSetting = 2;
            SetIsRunning(false);
        }
